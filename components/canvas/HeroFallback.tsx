 ...
 * so it sits directly on the page instead of inside a black box.
 *
 * An earlier version of this wrapped the logo in a separate, much larger
 * circular glow div — meant to echo the medallion's chrome disc, but in
 * practice it was just a generic circle with no relationship to the
 * logo's actual (roughly circular, but not exactly, and smaller) shape,
 * so it read as a mismatched ring sitting around the badge rather than
 * part of it. This drops that shape entirely: the only "glow" now is a
 * drop-shadow filter, which follows the logo artwork's real silhouette
 * (including its transparent gaps) rather than an unrelated geometric
 * shape layered behind it.
 *
 * It spins slowly via the `.spin-badge` class (see globals.css) — a cheap
 * CSS transform animation, a fraction of the cost of the real WebGL
 * scene, so this stays fine on exactly the low-power devices this
 * fallback exists for. Applied directly to the sizing div, not to a
 * Tailwind arbitrary-value animate-[...] utility — see the comment on
 * .spin-badge in globals.css for why that's a plain named class instead.
 *
 ...
 * solid shape reads as barely-there on line art, so this sits higher
 * (70%) than that comparison would suggest, to actually be legible.
 *
...
export function HeroFallback({ loading = false }: { loading?: boolean }) {
  return (
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden">
      ...
      <div className="spin-badge relative h-[54vw] w-[54vw] max-h-72 max-w-72">
        <Image
          src="/logo-badge.png"
          alt="Classic Cut Barber Shop logo"
          fill
          sizes="55vw"
          className="object-contain opacity-[0.7] drop-shadow-[0_0_26px_rgba(63,107,176,0.4)]"
          priority
        />
      </div>
